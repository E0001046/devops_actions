...
if int(p_SR_ETAPA) != 2:
    ...
    try:
        ...
        message = f"El catalogo oracle_dispersiones_{global_params['SR_FOLIO_REL']} no pudo ser encontrado, lo que significa que no se ha realizado la generacion de movimientos para el proceso de ABONO con dicho folio"
...
if int(p_SR_ETAPA) != 2:
    ...
    DF_DISPERSIONES_03.select("FTC_FOLIO", "FTC_FOLIO_REL").printSchema()
    ...
    DF_DISPERSIONES_03 = DF_DISPERSIONES_03.withColumn(
        "FTC_FOLIO", DF_DISPERSIONES_03["FTC_FOLIO"].cast("string")
    ).withColumn(
        "FTC_FOLIO_REL", DF_DISPERSIONES_03["FTC_FOLIO_REL"].cast("string")
    )

    # Realizar el left anti join para capturar los registros que no hacen match
    DF_250_REGS_ANT_02_5 = DF_DISPERSIONES_03.join(
        DF_260_REGS_ACTUALES_02_6,
        (DF_DISPERSIONES_03.FTC_FOLIO == DF_260_REGS_ACTUALES_02_6.FTC_FOLIO)
        & (DF_DISPERSIONES_03.FTC_FOLIO_REL == DF_260_REGS_ACTUALES_02_6.FTC_FOLIO_REL),
        how="left_anti"
    )
...
# Crea o actualiza la tabla escribiendo los datos
if int(p_SR_ETAPA) != 2:
    ...
    # Escribe una tabla vacía en modo "overwrite" para limpiar los datos sin borrar la estructura
    DF_UNION.limit(0).write.format("delta").mode("overwrite").saveAsTable(table_name)
    
    # Escribe los datos en la tabla
    DF_UNION.write.format("delta").mode("append").saveAsTable(table_name)
