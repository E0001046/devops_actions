...
if int(p_SR_ETAPA) != 2:
    ...
    DF_DISPERSIONES_03 = DF_DISPERSIONES_03.withColumn("FTN_ID_BAL_MOV", F.col("CONTADOR")) \
        .withColumn("FTC_FOLIO", F.col("FTC_FOLIO")) \
        .withColumn("FTC_FOLIO_REL", F.col("FTC_FOLIO_REL")) \
        .withColumn("FTN_DISP_PESOS", F.lit(0)) \
        .withColumn("FTN_DISP_ACCIONES", 
            F.when(F.lit(p_SR_TIPO_MOV) == 2, F.lit(0))
            .otherwise((F.col("FTF_MONTO_ACCIONES") * -1) * F.lit(p_SR_FACTOR))
        ) \
        .withColumn("FTN_PDTE_PESOS", 
            F.when(F.lit(p_SR_TIPO_MOV) == 2, F.col("FTF_MONTO_PESOS") * F.lit(p_SR_FACTOR))
            .otherwise(F.lit(0))
        ) \
        .withColumn("FTN_PDTE_ACCIONES", 
            F.when(F.lit(p_SR_TIPO_MOV) == 2, F.col("FTF_MONTO_ACCIONES") * F.lit(p_SR_FACTOR))
            .otherwise(F.lit(0))
        ) \
        .withColumn("FTN_COMP_PESOS", 
            F.when(F.lit(p_SR_TIPO_MOV) == 2, F.lit(0))
            .otherwise(F.col("FTF_MONTO_PESOS") * F.lit(p_SR_FACTOR))
        ) \
        .withColumn("FTN_COMP_ACCIONES", 
            F.when(F.lit(p_SR_TIPO_MOV) == 2, F.lit(0))
            .otherwise(F.col("FTF_MONTO_ACCIONES") * F.lit(p_SR_FACTOR))
        ) \
        ...
        .withColumn("FTN_ORIGEN_APORTACION", 
            F.when(F.col("FCN_ID_TIPO_SUBCTA").isin(19, 21, 23), F.lit(293))
        ...
        .withColumn("FCD_FEH_CRE", F.to_timestamp(from_utc_timestamp(current_timestamp(), "GMT-6"))) \
        .withColumn("FCC_USU_CRE", F.col("FCC_USU_CRE")) \
        .withColumn("FCD_FEH_ACT", F.to_timestamp(from_utc_timestamp(current_timestamp(), "GMT-6"))) \
...
# MAGIC %md
# MAGIC ### Guardamos el `DF_DISPERSIONES_03` como una tabla de unity catalog `ORACLE_DISPERSIONES`

# COMMAND ----------

if p_SR_ETAPA != 2:
    spark.sql(f"DROP TABLE IF EXISTS {global_confs['catalog_name']}.{global_confs['schema_name']}.oracle_dispersiones_{global_params['SR_FOLIO']}")
    DF_DISPERSIONES_03.write.format("delta").mode("overwrite").saveAsTable(f"{global_confs['catalog_name']}.{global_confs['schema_name']}.oracle_dispersiones_{global_params['SR_FOLIO']}")
