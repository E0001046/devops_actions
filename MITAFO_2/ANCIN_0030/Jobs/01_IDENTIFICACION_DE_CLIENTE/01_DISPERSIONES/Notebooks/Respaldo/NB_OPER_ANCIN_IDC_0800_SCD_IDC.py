...
# Directly repartition the DataFrame without checking the number of partitions
#df_nulos = df_nulos.repartition(100)

# Cachear si se usará varias veces
#df_nulos.cache()

#approx_count = df_nulos.count()
#display(approx_count)
catalog_name = "dbx_mit_dev_1udbvf_workspace"
schema_name = "default"
table_name = f"df_nulos"

#spark.sql(f"DROP TABLE IF EXISTS {catalog_name}.{schema_name}.{table_name}")
df_nulos.write.format("delta").mode("overwrite").saveAsTable(
