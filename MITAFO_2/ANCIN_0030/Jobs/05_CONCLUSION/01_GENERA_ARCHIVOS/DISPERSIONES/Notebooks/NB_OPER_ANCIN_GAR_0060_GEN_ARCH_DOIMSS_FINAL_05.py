...
if sr_subproceso == '8' and sr_subetapa=='428' and sr_tipo_archivo == '425':
    ...
    full_file_name = external_location + err_repo_path + "/SVIC1_" + fecha_actual + "_668.DAT"
else:
    logger.error("Parametros de entrada no validos para generar Archivos CTINDI")

try:
    df_final.write.format("csv").mode("overwrite").option("header", header).option("lineSep","\r\n").save(
        full_file_name + "_TEMP"
    )

    # Lee el archivo temporal en un nuevo DataFrame
    dataframe = spark.read.option("header", header).option("lineSep","\r\n").csv(full_file_name + "_TEMP")
...
# DBTITLE 1,CONSTRUCCIÓN DE ARCHIVO CSV
try:
...
# DBTITLE 1,CREACIÓN ARCHIVO TEMPORAL Y DEPURACIÓN
try:
...
#if sr_subproceso == '8' and sr_subetapa=='428' and sr_tipo_archivo == '425':
#    dbutils.fs.cp('abfss://nci-repository@datalakedev1udbvf.dfs.core.windows.net/RCDI/INTEGRITY/CTINDI1_' + fecha_actual + '_001.DAT',
#'/Volumes/dbx_mit_dev_1udbvf_workspace/default/doimss_carga_archivo/CTINDI1_' + fecha_actual + '_001.DAT')
    
#elif sr_subproceso == '9' and sr_subetapa=='428' and sr_tipo_archivo == '425':
#    dbutils.fs.cp('abfss://nci-repository@datalakedev1udbvf.dfs.core.windows.net/RCDI/INTEGRITY/SVIC1_' + fecha_actual + '_668.DAT',
#'/Volumes/dbx_mit_dev_1udbvf_workspace/default/doimss_carga_archivo/SVIC1_' + fecha_actual + '_668.DAT')
#else:
#    logger.error("No fue posisble copiar el archivo CTINDI")
