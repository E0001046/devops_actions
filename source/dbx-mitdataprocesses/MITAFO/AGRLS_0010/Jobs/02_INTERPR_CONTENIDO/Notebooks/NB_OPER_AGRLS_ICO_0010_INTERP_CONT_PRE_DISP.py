...
# MAGIC     return sql_conf_file, conn_schema_001, table_002, global_temp_view_schema_001, global_temp_view_004, '1'
# MAGIC
# MAGIC
...
# MAGIC
# MAGIC     process_name = 'root'
# MAGIC     sql_conf_file, conn_schema_001, table_002, global_temp_view_schema_001, global_temp_view_004, failed_task = conf_process_values(config_process_file, process_name)
# MAGIC     if failed_task == '0':
# MAGIC         logger.error("Please review log messages")
# MAGIC         notification_raised(webhook_url, -1, message, source, input_parameters)
# MAGIC         raise Exception("Process ends")
...
fnd_feh_cre = now.strftime('%d/%m/%Y T%H:%M:%S') + ('-%02d' % (now.microsecond / 10000))

query_statement = '005'

params = [temp_view_004, sr_folio, sr_proceso, sr_subproceso, sr_id_archivo]
