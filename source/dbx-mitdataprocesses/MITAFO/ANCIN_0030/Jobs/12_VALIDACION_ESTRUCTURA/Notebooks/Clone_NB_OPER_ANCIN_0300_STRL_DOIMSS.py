...
#Version alterna con delta
#df.write.format('delta').saveAsTable('people')

# COMMAND ----------

#Crean su vista normalmente
df.createOrReplaceTempView("TEST_LOCAL_VIEW")
#Crean una tabla
spark.sql("CREATE OR REPLACE TABLE TEST_LOCAL_TABLE AS SELECT * FROM TEST_LOCAL_VIEW")
#Borran la vista
spark.catalog.dropTempView("TEST_LOCAL_VIEW")
#Al final borrar la tabla que crearon
spark.sql("DROP TABLE TEST_LOCAL_TABLE")

# COMMAND ----------

spark.sql("SELECT * from TEST_LOCAL_table")
...
try:
    ...
    pdf["FTC_CONTROL"] = pdf.apply(lambda x : x[column_names[0]][0:2], axis = 1)
except Exception as e:
    logger.error("An error was raised: " + str(e))
    notification_raised(webhook_url, -1, message, source, input_parameters)
    raise Exception("Process ends")

dfAdition = spark.createDataFrame(pdf)

dfAdition = dfAdition.repartition(20)

del pdf

dfAdition.persist(StorageLevel.DISK_ONLY)

df, failed_task = aditional_values(dfAdition, sr_id_archivo, sr_subetapa)
...
temp_view = "TEMP_VALIDACION_ESTRUCTURA_ARCHIVO"

dfVal.persist(StorageLevel.DISK_ONLY)

try:
    dfVal.createOrReplaceTempView(temp_view)
except Exception as e:
    logger.error("An error was raised: " + str(e))
    notification_raised(webhook_url, -1, message, source, input_parameters)
    raise Exception("Process ends")
...
try:
    df_output_001.write.format("csv").mode('overwrite').option("header", header).save(full_file_name + '_TEMP')
    dataframe = spark.read.option('header', header).csv(full_file_name + '_TEMP')
    dataframe.coalesce(1).write.format('csv').mode('overwrite').option('header', header).save(full_file_name)
except Exception as e:
    logger.error("An error was raised: " + str(e))
    notification_raised(webhook_url, -1, message, source, input_parameters)
    raise Exception("Process ends")
