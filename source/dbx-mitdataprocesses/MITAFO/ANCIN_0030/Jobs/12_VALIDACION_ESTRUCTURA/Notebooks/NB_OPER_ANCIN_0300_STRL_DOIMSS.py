...
df_output_table.createOrReplaceTempView("TMP_VALIDACION_ESTRUCTURA_" + sr_id_archivo)

#display(df_output_table)

statement = f"""
SELECT * FROM TMP_VALIDACION_ESTRUCTURA_{sr_id_archivo}
UNION ALL
...
FROM TMP_VALIDACION_ESTRUCTURA_{sr_id_archivo}
	JOIN (SELECT COUNT(1) TOTAL FROM TMP_VALIDACION_ESTRUCTURA_{sr_id_archivo} WHERE FLC_DETALLE = '00') CEROCERO
		ON 1 = 1
	JOIN (SELECT SUM(FLN_TOTAL_ERRORES) TOTAL FROM TMP_VALIDACION_ESTRUCTURA_{sr_id_archivo} WHERE FLC_DETALLE <> '00' AND FLN_TOTAL_ERRORES > 0) ERRORES
		ON 1 = 1
WHERE 1 = 1
