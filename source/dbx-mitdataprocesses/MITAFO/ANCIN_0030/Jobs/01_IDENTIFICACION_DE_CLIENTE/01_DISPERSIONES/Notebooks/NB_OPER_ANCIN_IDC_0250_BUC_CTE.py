...
WITH UniqueRecords AS (
    SELECT 
        ...
        BUC.FTC_BANDERA,
        ROW_NUMBER() OVER (PARTITION BY BUC.FTC_NSS, BUC.FTN_NUM_CTA_INVDUAL ORDER BY CTAS.FTC_FOLIO) AS rn
    FROM
        CIERREN_ETL.TLSISGRAL_ETL_BUC BUC
    INNER JOIN 
        CIERREN_ETL.TTSISGRAL_ETL_DISPERSION_CTAS CTAS 
            ON LTRIM(BUC.FTC_NSS, '0') = LTRIM(CTAS.FTC_NSS_CURP, '0')
