...
for view in global_temp_views:
    view_name = view.name
    count_query = f"SELECT COUNT(*) AS count FROM global_temp.{view_name}"
    count_df = spark.sql(count_query)
    count = count_df.collect()[0]['count']
    view_counts.append((view_name, count))

view_counts_df = spark.createDataFrame(view_counts, ["view_name", "record_count"])
...
# MAGIC SELECT
# MAGIC     IDC.FTC_FOLIO,
# MAGIC     IDC.FTN_NSS_CURP,
# MAGIC     IDC.FTN_ID_ARCHIVO,
# MAGIC     IDC.FTN_NO_LINEA,
# MAGIC     IDC.FTN_NSS,
# MAGIC     IDC.FTC_CURP,
# MAGIC     NOIDEN.FLAG,
# MAGIC     NOIDEN.FTN_ESTATUS_DIAG
# MAGIC FROM global_temp.TEMP_IDEN_CLIENTE_29993 IDC
# MAGIC LEFT JOIN global_temp.TEMP_CLIENTE_NO_IDEN_29993 NOIDEN ON IDC.FTN_NSS_CURP = NOIDEN.FTN_NSS_CURP
# MAGIC WHERE NOIDEN.FLAG IS NULL OR NOIDEN.FTN_ESTATUS_DIAG = 0
