...
# MAGIC         #
# MAGIC         nss_curp = config.get(process_name, 'nss_curp')
# MAGIC     except Exception as e:
# MAGIC         logger.error("Function: %s", inspect.stack()[0][3])
# MAGIC         logger.error("An error was raised: " + str(e))
# MAGIC         return '0','0','0','0','0','0','0'
# MAGIC     return sql_conf_file, conn_schema_001, table_003, table_004, global_temp_view_006, nss_curp, '1'
# MAGIC
# MAGIC
...
# MAGIC
# MAGIC     process_name = 'root'
# MAGIC     sql_conf_file, conn_schema_001, table_003, table_004, global_temp_view_006, nss_curp, failed_task = conf_process_values(config_process_file, process_name)
# MAGIC     if failed_task == '0':
# MAGIC         logger.error("Please review log messages")
# MAGIC         notification_raised(webhook_url, -1, message, source, input_parameters)
# MAGIC         raise Exception("Process ends")
...
# DBTITLE 1,CREACIÓN VISTA GLOBAL TEMP_INDEN_BUC
temp_view = 'TEMP_INDEN_BUC' + '_' + sr_id_archivo

spark.catalog.dropGlobalTempView(temp_view)

df.createOrReplaceGlobalTempView(temp_view)

spark.sql(f"select * from global_temp.TEMP_INDEN_BUC_{sr_id_archivo}").show()
#spark.sql(f"select * from global_temp.TEMP_INDEN_BUC_{sr_id_archivo}").count()
