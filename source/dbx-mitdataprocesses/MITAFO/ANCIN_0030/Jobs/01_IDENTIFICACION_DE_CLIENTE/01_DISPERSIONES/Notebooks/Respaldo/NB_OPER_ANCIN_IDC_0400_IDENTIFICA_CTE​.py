...
if __name__ == "__main__":
    ...
    config_process_file = (
        root_repo
        + "/"
        + "ANCIN_0030/Jobs/01_IDENTIFICACION_DE_CLIENTE/01_DISPERSIONES/Conf/CF_PART_PROC.py.properties"
    )
...
vigentes_02_DF = spark.sql(
    f"SELECT * FROM global_temp.temp_vigentes_02_{sr_id_archivo}"
)
...
novigentes_03_DF = spark.sql(
    f"SELECT * FROM global_temp.temp_novigentes_03_{sr_id_archivo}"
)
