...
# MAGIC     return sql_conf_file, conn_schema_002,table_007,table_009,table_010,var_001,debug,catalog_name,schema_name, '1'
# MAGIC
# MAGIC
...
# MAGIC
# MAGIC     process_name = 'root'
# MAGIC     sql_conf_file, conn_schema_002,table_007,table_009,table_010,var_001,debug,catalog_name,schema_name,failed_task = conf_process_values(config_process_file, process_name)
# MAGIC     if failed_task == '0':
# MAGIC         logger.error("Please review log messages")
# MAGIC         notification_raised(webhook_url, -1, message, source, input_parameters)
# MAGIC         raise Exception("Process ends")
...
# DBTITLE 1,VALIDACIONES PARA GENERAR ARCHIVOS
stage = 0
# Contiene todos los registros del folio de la dispersión
if (sr_subproceso == '8') and (sr_subetapa == '428') and (sr_tipo_archivo == '425'): 
    stage = "CTINDI"
# Contiene solo aquellos registros que traen credito de vivienda para el folio de la dispersión
elif (sr_subproceso == '8') and (sr_subetapa == '978') and (sr_tipo_archivo == '426'):
    stage = "PPA_RCDI"
# Contiene los registros de vivienda de las diferentes dispersiones. Los que se hayan agrupado en el mes.
elif (sr_subproceso == '9') and (sr_subetapa == '428') and (sr_tipo_archivo == '425'):
    stage = "SVIC1"
else:
    logger.error("Parametros de entrada no validos para generar Archivos CTINDI")
    message = "Parametros de entrada no validos para generar Archivos CTINDI"
