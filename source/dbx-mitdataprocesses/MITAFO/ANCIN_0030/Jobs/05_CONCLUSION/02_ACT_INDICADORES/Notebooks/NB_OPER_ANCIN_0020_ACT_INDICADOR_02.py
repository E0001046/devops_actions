...
table_002 = 'ORACLE_DISPERSIONES'
table_003 = 'TEMP_DOIMSS_ACT_IND_01'

table_name_002 =  catalog_name + '.' + schema_name + '.' + table_002 + '_'+ sr_folio
table_name_003 = catalog_name + '.' + schema_name + '.' + table_003 + '_'+ sr_folio
...
params = [table_name_002, table_name_003]
...
# DBTITLE 1,EXTRACCIÓN ORACLE DISPERSIONES Y UNION CON IND
# Ejecuta la consulta SQL y obtiene el DataFrame resultante
df_mov_indi = spark.sql(statement)

temp_delta_002 = f"TEMP_DOIMSS_ACT_IND_02_{sr_folio}"
df_mov_indi.write.format("delta").mode("overwrite").saveAsTable(
    f"{catalog_name}.{schema_name}.{temp_delta_002}"
)

# Si está en modo debug True, muestra el DataFrame resultante
if debug:
    display(spark.table(f"{catalog_name}.{schema_name}.{temp_delta_002}"))
