...
# DBTITLE 1,CONSTRUCCIÓN DE CONSULTA PARA INDICADORES
#CIERREN.TTAFOGRAL_IND_CTA_INDV
table_name_002 = 'CIERREN_ETL.TTSISGRAL_ETL_MOVIMIENTOS'
...
# DBTITLE 1,EXTRACCIÓN DE INFORMACIÓN TABLA DELTA
df, failed_task = query_table(conn_name_ora, spark, statement,  conn_options, conn_user, conn_key)

temp_delta_001 = f"TEMP_DOIMSS_ACT_IND_01_{sr_folio}"
df.write.format("delta").mode("overwrite").saveAsTable(
    f"{catalog_name}.{schema_name}.{temp_delta_001}"
)

if debug:
    display(spark.table(f"{catalog_name}.{schema_name}.{temp_delta_001}"))

# COMMAND ----------

spark.table(f"{catalog_name}.{schema_name}.{temp_delta_001}").count()
