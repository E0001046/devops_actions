...
# DBTITLE 1,JP_PANCIN_MCV_0060_CIFRAS_CONTROL
query_statement = '008'

params = [sr_folio, sr_folio_rel, sr_proceso, sr_subproceso, 'DATABRICKS', sr_subetapa]
...
# DBTITLE 1,Delete CIFRAS_CTRL (Esquema CIERREN_ETL) Part 1
statement = f"""
DELETE FROM CIERREN_ETL.TLSISGRAL_ETL_VAL_CIFRAS_CTRL
WHERE FTC_FOLIO = '{sr_folio}'
AND     decode(FTC_FOLIO_REL,null,'-1',FTC_FOLIO_REL) = decode('{sr_folio_rel}','','-1','{sr_folio_rel}')
...
# DBTITLE 1,Delete CIFRAS_CTRL (Esquema CIERREN_ETL) Part 2
# MAGIC %scala
...
# DBTITLE 1,Delete CIFRAS_CTRL (Esquema CIERREN_ETL) Part 3
...
# DBTITLE 1,Delete CIFRAS_CTRL (Esquema CIERREN) Part 1
statement = f"""
DELETE FROM CIERREN.TLAFOGRAL_VAL_CIFRAS_CONTROL
WHERE FTC_FOLIO = '{sr_folio}'
AND     decode(FTC_FOLIO_REL,null,'-1',FTC_FOLIO_REL) = decode('{sr_folio_rel}','','-1','{sr_folio_rel}')
AND    FTC_ID_SUBETAPA = {sr_subetapa}
"""
...
# DBTITLE 1,Delete CIFRAS_CTRL (Esquema CIERREN) Part 2
# MAGIC %scala
# MAGIC import java.util.Properties
# MAGIC import java.sql.DriverManager
# MAGIC
...
# DBTITLE 1,Delete CIFRAS_CTRL (Esquema CIERREN) Part 3
#Manejo de errores de la operación realizada en Scala
...
# DBTITLE 1,Insert CIFRAS_CTRL (Esquema CIERREN_ETL)
mode = 'APPEND'
 
table_name_001 = 'CIERREN_ETL.TLSISGRAL_ETL_VAL_CIFRAS_CTRL'
...
# DBTITLE 1,Insert CIFRAS_CTRL (Esquema CIERREN)
mode = 'APPEND'
 
table_name_001 = 'CIERREN.TLAFOGRAL_VAL_CIFRAS_CONTROL'
...
# DBTITLE 1,Delete MONTOS_CLIENTE_AUX Part 1
statement = f"""
DELETE FROM CIERREN_DATAUX.TTSISGRAL_ETL_MONTOS_CLIENTE_AUX
WHERE FTC_FOLIO = '{sr_folio}'
"""
print(statement)
...
# DBTITLE 1,Delete MONTOS_CLIENTE_AUX Part 2
# MAGIC %scala
# MAGIC import java.util.Properties
# MAGIC import java.sql.DriverManager
# MAGIC
...
# DBTITLE 1,Delete MONTOS_CLIENTE_AUX Part 3
#Manejo de errores de la operación realizada en Scala
...
# DBTITLE 1,Elimina Vista TEMP_DOIMSS_MCV_MONTOS_CLIENTE
# Drop the temporary view if it exists
# spark.catalog.dropGlobalTempView('TEMP_DOIMSS_MCV_MONTOS_CLIENTE' + '_' + sr_folio)
